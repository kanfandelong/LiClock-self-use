...
private:
    ...
    {
        display.drawXBitmap(pos, y, weather_icons_day[code].data, weather_icons_day[code].width, weather_icons_day[code].height, TFT_BLACK);
    }
    // 在屏幕底部显示时间、降水、天气
    // 每格占37像素
    // 右边多打一个时间保证滚动显示效果
    void drawPlot(int hour = 0, int minute = 0)
    {
        ...
        display.fillRect(0, 122, SCREEN_WIDTH, 6, TFT_BLACK);
        ...
        // 绘制降水图表，纵坐标范围82-122
        display.fillRect(0, 122 - GRAPH_HEIGHT, SCREEN_WIDTH, GRAPH_HEIGHT, TFT_WHITE);
        for (int i = 0; i < SCREEN_WIDTH; ++i)
        {
            ...
            {
                display.drawPixel(i, 122 - ydata[i + minute_shift], TFT_BLACK);
                for (int j = 122 - ydata[i + minute_shift]; j < 122; j++)
                {
                    if ((i % 2) == (j % 2))
                        display.drawPixel(i, j, TFT_BLACK);
                }
            }
        }
        // 绘制天气图标
    ...
    void drawTime(int hour, int minute)
    {
        ...
        display.fillRect(7, 0, 86, 32, TFT_WHITE);
        display.printf("%02d:%02d", hour, minute);
        display.drawFastHLine(5, 30, 89, TFT_BLACK);
    }
    ...
    void drawDateAndDesc(int month, int date, int day)
    {
        ...
        display.drawFastHLine(5, 45, 89, TFT_BLACK);
    ...
    void drawFrame()
    {
        // display.drawLine(0, 82, SCREEN_WIDTH, 82, TFT_BLACK);
        display.fillRect(0, 96, 85, 200, TFT_WHITE); // 清空天气图标区域
        display.drawXBitmap(96, 2, weather_frames[0].data, weather_frames[0].width, weather_frames[0].height, TFT_BLACK);
        display.drawXBitmap(96 + 50, 2, weather_frames[1].data, weather_frames[1].width, weather_frames[1].height, TFT_BLACK);
        display.drawXBitmap(96 + 100, 2, weather_frames[2].data, weather_frames[2].width, weather_frames[2].height, TFT_BLACK);
        display.drawXBitmap(96 + 150, 2, weather_frames[3].data, weather_frames[3].width, weather_frames[3].height, TFT_BLACK);
