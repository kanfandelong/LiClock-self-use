...
void AppRadio::drawRadioUI(bool frequencyMode)
{
    ...
    // 进度条背景
    display.drawRect(10, display.height() - 20, 100, 12, GxEPD_BLACK);
    // 填充进度条（按百分比）
    display.fillRect(10, display.height() - 20, map(rssi, 0, 64, 0, 100), 12, GxEPD_BLACK);
    ...
    // 音量进度条背景
    display.drawRect(display.width() - 110, display.height() - 20, 100, 12, GxEPD_BLACK);
    // 填充进度条
    display.fillRect(display.width() - 110, display.height() - 20, map(volume, 0, 15, 0, 100), 12, GxEPD_BLACK);
...
void AppRadio::drawInfoPanel(int16_t x, int16_t y, int16_t w, int16_t h)
{
    ...
    {
        display.fillRect(x + 40 + i * 3, y + 10, 2, 10, GxEPD_BLACK);
    }
...
void AppRadio::drawProgressBar(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t percent)
{
    ...
    display.fillRect(x + 1, y + 1, fillWidth, h - 2, GxEPD_BLACK);
}
