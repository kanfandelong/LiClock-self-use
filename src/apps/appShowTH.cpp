...
#define MAX_POINTS SCREEN_WIDTH // 屏幕宽度限制
...
void AppShowTH::drawGraph(EnvData *data, int count)
{
    ...
    // 绘制坐标轴
    display.drawLine(0, MARGIN_TOP, SCREEN_WIDTH, MARGIN_TOP, TFT_BLACK);
    display.drawLine(0, SCREEN_HEIGHT - MARGIN_BOTTOM,
                     SCREEN_WIDTH, SCREEN_HEIGHT - MARGIN_BOTTOM, TFT_BLACK);

    // 添加中间分隔线以便区分温湿度区域
    display.drawLine(0, MARGIN_TOP + GRAPH_HEIGHT_ / 2,
                     SCREEN_WIDTH, MARGIN_TOP + GRAPH_HEIGHT_ / 2, TFT_BLACK);
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // display.drawPixel(x+1, y, TFT_BLACK);
        // display.drawPixel(x-1, y, TFT_BLACK);
        // display.drawPixel(x, y+1, TFT_BLACK);
        // display.drawPixel(x, y-1, TFT_BLACK);
    }
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // display.drawPixel(x+1, y, TFT_BLACK);
        // display.drawPixel(x, y+1, TFT_BLACK);
        // display.drawPixel(x+1, y+1, TFT_BLACK);
    }
