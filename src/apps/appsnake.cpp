...
void Appsnake::drawGame()
{
  ...
  for (uint8_t i = 0; i < snakeLength; i++)
  {
    display.fillRect((snakeshuzu[i].x * snakeSize) + wallOffsetX, (snakeshuzu[i].y * snakeSize) + wallOffsetY, snakeSize, snakeSize, TFT_BLACK);
  }

  // 画食物
  display.fillRect((food.x * snakeSize) + wallOffsetX, (food.y * snakeSize) + wallOffsetY, snakeSize, snakeSize, TFT_BLACK);
...
void Appsnake::setup()
{
  ...
  display.fillScreen(TFT_WHITE);
  logoY += moveStep;
  // display.drawInvertedBitmap(logoX, logoY, logo, 143, 66, TFT_BLACK);
