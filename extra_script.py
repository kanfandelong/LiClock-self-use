...
def get_git_info():
    ...
    try:
        # 完整哈希
        hash_full = subprocess.check_output(
            ['git', 'rev-parse', 'HEAD'],
...
def generate_git_info_header():
    """生成 include/git_info.h 头文件（仅内容变化时更新）"""
    git_info = get_git_info()
    header_content = f"""// Auto-generated by PlatformIO build script
// DO NOT EDIT MANUALLY

#ifndef GIT_INFO_H
#define GIT_INFO_H

#define GIT_COMMIT_HASH_FULL "{git_info['hash_full']}"
#define GIT_COMMIT_HASH_SHORT "{git_info['hash_short']}"
#define GIT_BRANCH "{git_info['branch']}"
#define GIT_DESCRIBE "{git_info['describe']}"
#define GIT_IS_DIRTY {git_info['is_dirty']}

#endif // GIT_INFO_H
"""
    # 目标头文件路径（项目 include 目录）
    include_dir = os.path.join(env.subst("$PROJECT_DIR"), "include")
    os.makedirs(include_dir, exist_ok=True)
    header_path = os.path.join(include_dir, "git_info.h")
    ...
    if need_write:
        ...
        print("✅ 已生成 Git 版本头文件: include/git_info.h")
        print(f"   Git commit: {git_info['hash_short']} ({git_info['branch']})")
    else:
        print("⏭️ Git 信息未变化，跳过头文件更新")
