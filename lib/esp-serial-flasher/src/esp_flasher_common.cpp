...
esp_loader_error_t connect_to_target(uint32_t higher_transmission_rate)
{
    ...
    if (err != ESP_LOADER_SUCCESS) {
        log_e("Cannot connect to target. Error: %s", get_error_string(err));

        if (err == ESP_LOADER_ERROR_TIMEOUT) {
            log_e("Check if the host and the target are properly connected.");
        } else if (err == ESP_LOADER_ERROR_INVALID_TARGET) {
            log_e("You could be using an unsupported chip, or chip revision.");
        } else if (err == ESP_LOADER_ERROR_INVALID_RESPONSE) {
            log_e("Try lowering the transmission rate or using shorter wires to connect the host and the target.");
        }

        return err;
    }
    log_i("Connected to target\n");
    ...
    if (higher_transmission_rate && esp_loader_get_target() != ESP8266_CHIP) {
        ...
        if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
            log_w("ESP8266 does not support change transmission rate command.");
            return err;
        } else if (err != ESP_LOADER_SUCCESS) {
            log_e("Unable to change transmission rate on target.");
            return err;
        } else {
            err = loader_port_change_transmission_rate(higher_transmission_rate);
            if (err != ESP_LOADER_SUCCESS) {
                log_e("Unable to change transmission rate.");
                return err;
            }
            log_i("Transmission rate changed.");
        }
    }
...
esp_loader_error_t flash_binary(const uint8_t *bin, size_t size, size_t address)
{
    ...
    log_i("Erasing flash (this may take a while)...");
    err = esp_loader_flash_start(address, size, sizeof(payload));
    if (err != ESP_LOADER_SUCCESS) {
        log_e("Erasing flash failed with error: %s.", get_error_string(err));

        if (err == ESP_LOADER_ERROR_INVALID_PARAM) {
            log_w("If using Secure Download Mode, double check that the specified "
    ...
    while (size > 0) {
        ...
        if (err != ESP_LOADER_SUCCESS) {
            log_e("Packet could not be written! Error %s.", get_error_string(err));
            return err;
        }
    ...
    log_i("Finished programming");

#if MD5_ENABLED
    err = esp_loader_flash_verify();
    if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
        log_w("ESP8266 does not support flash verify command.");
        return err;
    } else if (err != ESP_LOADER_SUCCESS) {
        log_e("MD5 does not match. Error: %s", get_error_string(err));
        return err;
    }
    log_i("Flash verified");
#endif
...
esp_loader_error_t load_ram_binary(const uint8_t *bin)
{
    ...
    for (seg = 0; seg < header->segments; seg++) {
        log_i("Downloading %"PRIu32" bytes at 0x%08"PRIx32"...", segments[seg].size, segments[seg].addr);

        err = esp_loader_mem_start(segments[seg].addr, segments[seg].size, ESP_RAM_BLOCK);
        if (err != ESP_LOADER_SUCCESS) {
            log_e("Loading to RAM could not be started. Error: %s.\n", get_error_string(err));

            if (err == ESP_LOADER_ERROR_INVALID_PARAM) {
                log_e("Check if the chip has Secure Download Mode enabled.\n");
            }
            return err;
        }
        ...
        while (remain_size > 0) {
            ...
            if (err != ESP_LOADER_SUCCESS) {
                log_i("Packet could not be written! Error: %s.", get_error_string(err));
                return err;
            }
            data_pos += data_size;
    ...
    if (err != ESP_LOADER_SUCCESS) {
        log_e("Loading to RAM finished with error: %s.", get_error_string(err));
        return err;
    }
    log_i("Finished loading");
