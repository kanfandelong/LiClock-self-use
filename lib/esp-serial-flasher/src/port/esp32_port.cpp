...
esp_loader_error_t loader_port_esp32_init(const loader_esp32_config_t *config)
{
    ...
    if (!config->dont_initialize_peripheral) {
        ...
        if ( gpio_set_drive_capability((gpio_num_t)config->uart_tx_pin, GPIO_DRIVE_CAP_3) != ESP_OK ) {
    ...
    gpio_reset_pin((gpio_num_t)s_reset_trigger_pin);
    gpio_set_pull_mode((gpio_num_t)s_reset_trigger_pin, GPIO_PULLUP_ONLY);
    gpio_set_direction((gpio_num_t)s_reset_trigger_pin, GPIO_MODE_OUTPUT);

    gpio_reset_pin((gpio_num_t)s_gpio0_trigger_pin);
    gpio_set_pull_mode((gpio_num_t)s_gpio0_trigger_pin, GPIO_PULLUP_ONLY);
    gpio_set_direction((gpio_num_t)s_gpio0_trigger_pin, GPIO_MODE_OUTPUT);
...
{
    gpio_set_level((gpio_num_t)s_gpio0_trigger_pin, SERIAL_FLASHER_BOOT_INVERT ? 1 : 0);
    loader_port_reset_target();
    loader_port_delay_ms(SERIAL_FLASHER_BOOT_HOLD_TIME_MS);
    gpio_set_level((gpio_num_t)s_gpio0_trigger_pin, SERIAL_FLASHER_BOOT_INVERT ? 0 : 1);
}


void loader_port_reset_target(void)
{
    gpio_set_level((gpio_num_t)s_reset_trigger_pin, SERIAL_FLASHER_RESET_INVERT ? 1 : 0);
    loader_port_delay_ms(SERIAL_FLASHER_RESET_HOLD_TIME_MS);
    gpio_set_level((gpio_num_t)s_reset_trigger_pin, SERIAL_FLASHER_RESET_INVERT ? 0 : 1);
}
