...
#define BQ72441_I2C_TIMEOUT 250

// current() 函数的参数，用于指定要读取的电流类型
typedef enum {
	AVG,  // 平均电流（默认）
	STBY, // 待机电流
	MAX   // 最大电流
...
typedef enum {
	BATTERY,      // 电池温度（默认）
	INTERNAL_TEMP // 内部IC温度
} temp_measure;

// setGPOUTFunction() 函数的参数
typedef enum {
	SOC_INT, // 设置 GPOUT 为 SOC_INT 功能
	BAT_LOW  // 设置 GPOUT 为 BAT_LOW 功能
} gpout_function;
