...
size_t Preferences::getBytes(const char *key, void *buf, size_t maxLen)
{
    ...
    esp_err_t err;
    if (len > maxLen)
        err = nvs_get_blob(_handle, key, buffer, &len);
    else
    ...
    if (len > maxLen)
    {
        log_w("not enough space in buffer: %u < %u,the data might get cut off.", maxLen, len);
...
nvs_stats_t Preferences::getStats()
{
    nvs_stats_t nvs_stats = {0, 0, 0, 0};
    esp_err_t err = nvs_get_stats(NULL, &nvs_stats);
    if (err)
    {
        log_e("Failed to get nvs statistics");
