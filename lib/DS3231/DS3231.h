...
class DS3231 {
	...
	void getAll()
	{
		...
		// Alarm functions
		//A1Dy:0:Day为日期,1:Day为星期
		void getA1Time(byte& A1Day, byte& A1Hour, byte& A1Minute, byte& A1Second, byte& AlarmBits, bool& A1Dy); 
 ...
 */
		...
		/**
		 * @brief 读取RTC晶振偏移量, 1lsb=0.1ppm@25°C
		 * @return 一个有符号整数，代表当前RTC偏移量
		 */
		int8_t readOffset();
		/**
		 * @brief 写入RTC晶振偏移量, 1lsb=0.1ppm@25°C
		 */
		void writeOffset(int8_t offset);
