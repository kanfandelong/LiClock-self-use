...
void DS3231::getA1Time(byte& A1Day, byte& A1Hour, byte& A1Minute, byte& A1Second, byte& AlarmBits, bool& A1Dy) {
	byte temp_buffer;
	Wire.beginTransmission(CLOCK_ADDRESS);
	Wire.write(0x07);
	Wire.endTransmission();
...
void DS3231::writeOffset(int8_t offset) {
	Wire.beginTransmission(CLOCK_ADDRESS);
	Wire.write(0x10);
	Wire.write(offset);
	Wire.endTransmission();
