...
class GxEPD2_BW : public GxEPD2_GFX_BASE_CLASS
{
  private:
    uint8_t *_buffer;
    uint8_t _buffers[3][(GxEPD2_Type::WIDTH / 8) * page_height];
  public:
    GxEPD2_Type epd2;
  ...
  void swapBuffer(uint16_t buffer_index)
  {
    ...
    _buffer = _buffers[buffer_index];
  }
  void copyBuffer(uint16_t to, uint16_t from)
  {
    if(from > 2 || to > 2) return;
    memcpy(_buffers[to], _buffers[from], (GxEPD2_Type::WIDTH / 8) * page_height);
  }
#if ENABLE_GxEPD2_GFX
    ...
    void fillScreen(uint16_t color) // 0x0 black, >0x0 white, to buffer
    {
      ...
      for (uint16_t x = 0; x < (GxEPD2_Type::WIDTH / 8) * page_height; x++)
      {
        _buffer[x] = data;
      }
    }
