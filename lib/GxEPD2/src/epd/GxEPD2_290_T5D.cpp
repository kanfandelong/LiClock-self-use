...
GxEPD2_290_T5D::GxEPD2_290_T5D(int16_t cs, int16_t dc, int16_t rst, int16_t busy) : GxEPD2_EPD(cs, dc, rst, busy, LOW, 10000000, WIDTH, HEIGHT, panel, hasColor, hasPartialUpdate, hasFastPartialUpdate)
{
}
...
void GxEPD2_290_T5D::SendLuts(uint8_t LutLevel)
{
    ...
    for (uint8_t i = 0; i < 5; i++)
    {
        ...
        {
            if (j == 4 && ((i == 2) || (greyHQ == 3 && i == 4)))
                _writeData(0x0f); // 刷黑->白
            else if (j == greyHQ)
                _writeData(LutLevel);
            else if (j == 0)
                _writeData(pgm_read_byte(lutFast_ + (i)));
            else if (j == 5)
                _writeData(1);
            else
                _writeData(0x0);
...
{
    _writeCommand(0x12); // display refresh
    _waitWhileBusy("_Update_Part", T5D ? 750 : 450);
}
