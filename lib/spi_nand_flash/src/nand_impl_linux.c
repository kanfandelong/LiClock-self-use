...
esp_err_t nand_is_bad(spi_nand_flash_device_t *handle, uint32_t block, bool *is_bad_status)
{
    ...
    ESP_LOGD(TAG, "is_bad, block=%lu, page=%lu,indicator = %04x", block, block_offset, bad_block_indicator);
...
{
    ESP_LOGV(TAG, "read, page=%lu, offset=%ld, length=%ld", page, offset, length);
