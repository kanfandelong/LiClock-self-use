                                 ...
                                 const esp_vfs_fat_mount_config_t *mount_config)
{
    ...
    if (fresult != FR_OK)
    {
        ...
        if (!((fresult == FR_NO_FILESYSTEM || fresult == FR_INT_ERR) && mount_config->format_if_mount_failed))
        {
            ret = ESP_FAIL;
            goto fail;
        }
...
#else
        BYTE opt = FM_ANY;
        DWORD au = alloc_unit_size;
        fresult = f_mkfs(drv, opt, au, workbuf, workbuf_size);
#endif
        if (fresult != FR_OK)
        {
            ret = ESP_FAIL;
