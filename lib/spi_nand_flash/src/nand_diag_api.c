...
esp_err_t nand_get_bad_block_stats(spi_nand_flash_device_t *flash, uint32_t *bad_block_count)
{
    ...
    for (uint32_t blk = 0; blk < num_blocks; blk++) {
        ...
        if (ret == ESP_OK && is_bad) {
            bad_blocks++;
            ESP_LOGD(TAG, "bad block num=%lu", blk);
        } else if (ret) {
            ESP_LOGE(TAG, "Failed to get bad block status for blk=%lu", blk);
            return ret;
        }
    }
...
esp_err_t nand_get_ecc_stats(spi_nand_flash_device_t *flash)
{
    ...
    for (uint32_t page = 0; page < num_pages; page++) {
        ...
        if (!is_free) {
            ...
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to read ecc error for page=%lu", page);
                return ret;
            }
            if (flash->chip.ecc_data.ecc_corrected_bits_status) {
                ...
                if (flash->chip.ecc_data.ecc_corrected_bits_status == STAT_ECC_NOT_CORRECTED) {
                    ...
                    ESP_LOGD(TAG, "ecc error not corrected for page=%lu", page);
    ...
    ESP_LOGI(TAG, "\nTotal number of ECC errors: %lu\nECC not corrected count: %lu\nECC errors exceeding threshold (%d): %lu\n",
