...
#include "esp_check.h"
#include "esp_err.h"
#include "../priv_include/spi_nand_oper.h"
#include "spi_nand_flash.h"
#include "../priv_include/nand.h"
...
esp_err_t nand_is_bad(spi_nand_flash_device_t *handle, uint32_t block, bool *is_bad_status)
{
    ...
    ESP_LOGD(TAG, "is_bad, block=%lu, page=%lu,indicator = %04x", block, first_block_page, bad_block_indicator);
...
esp_err_t nand_mark_bad(spi_nand_flash_device_t *handle, uint32_t block)
{
    ...
    uint8_t status;
    ESP_LOGD(TAG, "mark_bad, block=%lu, page=%lu,indicator = %04x", block, first_block_page, bad_block_indicator);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s: mark_bad write verification failed for block=%lu and page=%lu", __func__, block, first_block_page);
    }
#endif //CONFIG_NAND_FLASH_VERIFY_WRITE
    return ret;
fail:
...
{
    ESP_LOGD(TAG, "erase_block, block=%lu,", block);
    esp_err_t ret = ESP_OK;
    uint8_t status;
...
{
    ESP_LOGV(TAG, "prog, page=%lu,", page);
    esp_err_t ret = ESP_OK;
    ...
    if ((status & STAT_PROGRAM_FAILED) != 0) {
        ESP_LOGD(TAG, "prog failed, page=%lu,", page);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s: prog page=%lu write verification failed", __func__, page);
    }
    ret = s_verify_write(handle, (uint8_t *)&used_marker, column_addr + handle->chip.page_size + 2, 2);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s: prog page=%lu used marker write verification failed", __func__, page);
    }
#endif //CONFIG_NAND_FLASH_VERIFY_WRITE
...
esp_err_t nand_is_free(spi_nand_flash_device_t *handle, uint32_t page, bool *is_free_status)
{
    ...
    ESP_LOGD(TAG, "is free, page=%lu, used_marker=%04x,", page, used_marker);
...
{
    ESP_LOGV(TAG, "read, page=%lu, offset=%d, length=%d", page, offset, length);
    ...
    if (is_ecc_error(handle, status)) {
        ESP_LOGD(TAG, "read ecc error, page=%lu", page);
        return ESP_FAIL;
    }
...
{
    ESP_LOGD(TAG, "copy, src=%lu, dst=%lu", src, dst);
    esp_err_t ret = ESP_OK;
#if CONFIG_NAND_FLASH_VERIFY_WRITE
    uint8_t *temp_buf = NULL;
#endif //CONFIG_NAND_FLASH_VERIFY_WRITE
    ...
    if (src_column_addr != dst_column_addr) {
        ...
        ESP_GOTO_ON_ERROR(read_page_and_wait(handle, src, &status), fail, TAG, "");
        if (is_ecc_error(handle, status)) {
            ESP_LOGE(TAG, "%s: dst_page=%lu read, ecc error", __func__, dst);
            goto fail;
        }
    }
    ...
    if (spi_nand_read(handle, temp_buf, src_column_addr, handle->chip.page_size)) {
        ESP_LOGE(TAG, "%s: Failed to read src_page=%lu", __func__, src);
        goto fail;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s: dst_page=%lu write verification failed", __func__, dst);
    }
